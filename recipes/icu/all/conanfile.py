...
class ICUBase(ConanFile):
    ...
    def build(self):
        ...
        if self._is_msvc:
            ...
            if self.settings.build_type in ["Debug", "RelWithDebInfo"] and tools.Version(
    ...
    def _configure_autotools(self):
        ...
        if "msys2" in self.deps_user_info:
            self._env_build.vars["PYTHON"] = tools.unix_path(
                os.path.join(self.deps_env_info["msys2"].MSYS_BIN, "python"), tools.MSYS2)
        return self._env_build
    ...
    def package_info(self):
        ...
        self.cpp_info.components["icu-data"].libs = [
            self._lib_name("icudt" if self.settings.os == "Windows" else "icudata")]
        if not self.options.shared:
        ...
        self.cpp_info.components["icu-i18n"].libs = [
            self._lib_name("icuin" if self.settings.os == "Windows" else "icui18n")]
