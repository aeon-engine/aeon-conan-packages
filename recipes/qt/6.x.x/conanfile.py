...
class QtConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.options.openssl:
            self.requires(self.conan_data["dependencies"][self.version]["openssl"])
        if self.options.with_pcre2:
            self.requires(self.conan_data["dependencies"][self.version]["pcre2"])
        ...
        if self.options.get_safe("with_freetype", False) and not self.options.multiconfiguration:
            self.requires(self.conan_data["dependencies"][self.version]["freetype"])
        ...
        if self.options.get_safe("with_libjpeg", False) and not self.options.multiconfiguration:
            self.requires(self.conan_data["dependencies"][self.version]["libjpeg-turbo"])
    ...
    def build_requirements(self):
        self.build_requires(self.conan_data["dependencies"][self.version]["cmake"])
        self.build_requires(self.conan_data["dependencies"][self.version]["ninja"])
    ...
    def source(self):
        ...
        shutil.move("qt-%s" % self.version, "qt6")
